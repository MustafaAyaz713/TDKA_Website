<section class="pricing-table section" style="background-color: #fff">
    <div class="container">
      <div class="row">
        <div class="col-lg-12 col-gl-4">
          <div class="section-title">
            <h2><br> <br> Güçlü iş süreçleri için <span style="color:#343A40;">TDKA Studios</span>
              yanınızda
            </h2>
            <span class="line"></span>
            <p></p>
          </div>
        </div>
      </div>
      <div class="row">
        <div class="col-lg-4 col-md-12 col-12 col-gl-4">
          <div class="single-table">
            <div class="table-head">
              <div class="icon">
                <i class="fa-sharp fa-solid fa-clipboard-list"></i>
              </div>
              <h4 class="title"></h4>
              <div class="price">
                <p class="amount">Planlama<span></span></p>
            </div>
          </div>
          <ul class="table-list">
            <li>
              <strong>
                Puantajı tek ekranda takip edin
            </li>
            <li>İzin süreçlerini kolayca yönetin</li>
            <li><br>Çalışan bilgilerini tek bir yerde toplayın<br></strong></li>
        </div>
      </div>
      <div class="col-lg-4 col-md-12 col-12 col-gl-4">
        <div class="single-table">
          <div class="table-head">
            <div class="icon">
              <i class="fa-solid fa-user-check"></i>
            </div>
            <h4 class="title"></h4>
            <div class="price">
              <p class="amount">Takip Etme<span></span></p>
            </div>
          </div>
          <ul class="table-list">
            <li>
              <strong>
                tıkla tüm vardiyayı düzene sokun<br>
            </li>
            <li>Bilgilerinizi tek tıkla raporlayın</li>
            <li></i> <br> Performans değerlendirme raporları <br><br> </strong></li>
          </ul>
        </div>
      </div>
      <div class="col-lg-4 col-md-12 col-12 col-gl-4">
        <div class="single-table">
          <div class="table-head">
            <div class="icon">
              <i class="fa-solid fa-sitemap"></i>
            </div>
            <h4 class="title"></h4>
            <div class="price">
              <p class="amount">Organizasyon<span></span></p>
            </div>
          </div>
          <ul class="table-list">
            <li>
              <strong>
                Kullanıcı Yönetimi ve Yetkilendirme
            </li>
            <li>İşe Alım ve İşe Yerleştirme Yönetimi</li>
            <li></i><br>İş Sağlığı Güvenliği ve SGK Belge Dökümleri Yönetimi</strong></li>
          </ul>
          </div>
        </div>
      </div>
    </div>
  </section>
  <p> <br> <br> <br> </p>
